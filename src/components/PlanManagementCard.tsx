...
export const PlanManagementCard: React.FC<PlanManagementCardProps> = ({
  ...
  return (
    <div className="space-y-6">
      {/* Feedback Messages */}
      {saveSuccessMsg && (
        <div className="flex items-center gap-2 p-4 bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-300 dark:border-emerald-800 rounded-2xl text-xs font-bold text-emerald-800 dark:text-emerald-200 animate-fade-in shadow-xs">
                  ...
                  {isEditing ? (
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        autoFocus
                        className="px-3 py-1.5 text-sm font-bold bg-white dark:bg-slate-900 border border-emerald-500 rounded-xl text-slate-900 dark:text-white focus:outline-none"
                      />
                      <button
                ...
                <div className="flex items-center gap-2 shrink-0 self-end sm:self-center">
                  {!isActive ? (
                    <button
                      ...
                      className="px-3.5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-xl transition-all cursor-pointer shadow-2xs flex items-center gap-1.5"
                    >
                      <Check className="w-3.5 h-3.5" />
                      <span>Switch to Plan</span>
                    </button>
                  ) : (
                    <span className="px-3 py-1.5 bg-emerald-100 dark:bg-emerald-900/40 text-emerald-800 dark:text-emerald-300 text-xs font-bold rounded-xl flex items-center gap-1.5">
                      <Check className="w-3.5 h-3.5" />
                      <span>Loaded</span>
                    </span>
                  )}
      ...
      <div id="card-plan-mgmt-json" className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-3xl p-6 shadow-sm space-y-5">
        ...
        {importSuccessMsg && (
          <div className="p-3 bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-300 dark:border-emerald-800 rounded-2xl text-xs font-bold text-emerald-800 dark:text-emerald-200">
            {importSuccessMsg}
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="p-5 rounded-2xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200/70 dark:border-slate-700/60 space-y-3 flex flex-col justify-between">
            ...
            <label className="w-full py-2.5 px-4 bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs rounded-xl transition-all cursor-pointer shadow-xs flex items-center justify-center gap-2">
