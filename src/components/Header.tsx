      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 h-full flex items-center justify-between gap-3">
        ...
        <div className="relative py-1 min-w-0 flex-1 max-w-xl flex items-center gap-2" ref={dropdownRef}>
          <button
            ...
            className="group flex items-center gap-2.5 px-3 py-1.5 rounded-xl bg-slate-50 dark:bg-slate-800/80 hover:bg-slate-100 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-700/80 transition-all text-left w-full sm:w-auto cursor-pointer focus:outline-none focus:ring-2 focus:ring-primary-500/30"
            aria-expanded={isOpen}
            aria-haspopup="listbox"
          >
            <div className="w-6 h-6 sm:w-7 sm:h-7 bg-primary-600 dark:bg-primary-500/20 border border-primary-500/30 rounded-lg flex items-center justify-center text-white dark:text-primary-400 shadow-xs shrink-0">
              <span className="w-2 h-2 rounded-full bg-primary-400 animate-pulse" />
            </div>

            <div className="flex items-center gap-2 min-w-0 flex-1">
              <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-primary-700 dark:text-primary-400 shrink-0 bg-primary-100 dark:bg-primary-950/80 px-2 py-0.5 rounded-full border border-primary-200 dark:border-primary-800">
                Active Plan
              </span>
          ...
          {onSaveScenario && (
            <button
              ...
              className="flex items-center justify-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-primary-50 dark:bg-primary-950/50 hover:bg-primary-100 dark:hover:bg-primary-900/60 text-primary-700 dark:text-primary-400 border border-primary-200 dark:border-primary-800 hover:border-primary-400 transition-all shadow-2xs hover:shadow-xs cursor-pointer group shrink-0"
              title={`Save Plan ("${activeScenario ? activeScenario.name : 'Active Plan'}")`}
              aria-label="Save active plan"
            >
              <Save className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-primary-600 dark:text-primary-400 transition-transform group-hover:scale-110" />
              <span className="hidden sm:inline text-xs font-bold">Save</span>
            </button>
          )}

                      {/* Quick New Plan Action Icon Button */}
            {onNewScenario && (
              <button
                type="button"
                onClick={(e) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 rounded-xl text-xs flex items-center justify-between gap-2 transition-colors cursor-pointer ${
                        isActive
                          ? 'bg-primary-50 dark:bg-primary-950/60 text-primary-900 dark:text-primary-200 font-bold border border-primary-200 dark:border-primary-800/60'
              ...
              {onNewScenario && (
                <div className="p-1.5 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30">
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold text-primary-700 dark:text-primary-400 bg-primary-50 dark:bg-primary-950/50 hover:bg-primary-100 dark:hover:bg-primary-900/60 border border-primary-200 dark:border-primary-800 transition-colors cursor-pointer"
                  >
