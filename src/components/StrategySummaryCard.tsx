...
export const StrategySummaryCard: React.FC<StrategySummaryCardProps> = ({
  ...
  return (
    <div className="space-y-4">
      <div className="bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 text-white rounded-2xl p-4 sm:p-5 border border-indigo-900/60 shadow-lg space-y-4">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-indigo-800/50">
        ...
        <div className="flex items-center gap-3">
          <div className="text-right">
            <span className="text-[10px] font-bold text-indigo-300 block uppercase tracking-wider">Target Household Income</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-xs">
        ...
        <div className="bg-slate-800/80 p-4 rounded-xl border border-slate-700/80 space-y-2.5">
          <div className="flex items-center justify-between border-b border-slate-700 pb-2">
            ...
            <span className="text-[10px] font-extrabold text-emerald-400 bg-emerald-950/80 px-2 py-0.5 rounded-md">
              Retire @ Age {profile.targetRetirementAge}
            </span>
          </div>
          <div className="space-y-1.5 text-[11px] text-slate-300">
            <div className="flex justify-between">
              <span className="text-slate-400">Product Option:</span>
            ...
            <div className="flex justify-between">
              <span className="text-slate-400">Drawdown Ordering:</span>
            ...
            {profile.incomeProductOption === 'hybrid' && (profile.annuityTranches || []).filter(t => t.enabled && (!t.owner || t.owner === 'primary')).length > 0 && (
              <div className="pt-1 border-t border-slate-700/60 space-y-1">
                <span className="text-slate-400 font-semibold block">Annuity Tranches:</span>
                {(profile.annuityTranches || []).filter(t => t.enabled && (!t.owner || t.owner === 'primary')).map((t, i) => (
                  <div key={t.id} className="flex justify-between pl-2">
                    <span className="text-slate-400">{t.name || `Tranche ${i + 1}`} (Age {t.purchaseAge}):</span>
                    <span className="font-bold text-amber-300">{t.allocationPercent}% @ {t.annuityRatePercent}% rate</span>
                  </div>
                ))}
              </div>
            )}
            <div className="flex justify-between">
              <span className="text-slate-400">PCLS Tax-Free Cash Taken:</span>
              <span className="font-bold text-emerald-400">
                £{primaryPclsInfo.taxFreeCashTaken.toLocaleString()} ({primaryPclsInfo.pclsPercent}% at age {primaryPclsInfo.takeAge})
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">PCLS Access Age:</span>
              <span className="font-bold text-white font-mono">Age {primaryPclsInfo.takeAge}</span>
            </div>
          </div>
        </div>
        ...
        {isCouple && partnerPclsInfo ? (
          <div className="bg-slate-800/80 p-4 rounded-xl border border-indigo-800/60 space-y-2.5">
            <div className="flex items-center justify-between border-b border-indigo-800/60 pb-2">
              ...
              <span className="text-[10px] font-extrabold text-indigo-300 bg-indigo-950/80 px-2 py-0.5 rounded-md">
            ...
            <div className="space-y-1.5 text-[11px] text-slate-300">
              ...
              {(profile.partnerIncomeProductOption || profile.incomeProductOption) === 'hybrid' && (profile.partnerAnnuityTranches || profile.annuityTranches || []).filter(t => t.enabled && (!t.owner || t.owner === 'partner')).length > 0 && (
                <div className="pt-1 border-t border-indigo-700/60 space-y-1">
                  <span className="text-slate-400 font-semibold block">Annuity Tranches:</span>
              ...
              <div className="flex justify-between">
                <span className="text-slate-400">PCLS Access Age:</span>
                <span className="font-bold text-white font-mono">Age {partnerPclsInfo.takeAge}</span>
              </div>
            </div>
          </div>
        ) : (
          <div className="bg-slate-800/40 p-4 rounded-xl border border-slate-700/50 flex flex-col items-center justify-center text-center space-y-1.5">
            <User className="w-6 h-6 text-slate-500 opacity-60" />
            <span className="font-bold text-slate-300">Single Planning Mode</span>
        ...
        <div className="bg-slate-800/80 p-4 rounded-xl border border-slate-700/80 space-y-2.5">
          ...
          <div className="space-y-1.5 text-[11px] text-slate-300">
            <div className="flex justify-between">
              <span className="text-slate-400">{profile.name || 'Primary'} SP Age:</span>
              <span className="font-bold text-white">Age {profile.statePensionAge || 67}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">{profile.name || 'Primary'} Entitlement:</span>
              <span className="font-bold text-emerald-400">
            ...
            {isCouple && (
              <>
                <div className="flex justify-between border-t border-slate-700/60 pt-1">
                  <span className="text-slate-400">{profile.partnerName || 'Partner'} SP Age:</span>
                  <span className="font-bold text-white">Age {profile.partnerStatePensionAge || 67}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400">{profile.partnerName || 'Partner'} Entitlement:</span>
                  <span className="font-bold text-indigo-300">
            ...
            <div className="flex justify-between border-t border-slate-700/60 pt-1">
              <span className="text-slate-400">Inflation Indexing:</span>
