...
export const PotTransferManager: React.FC<PotTransferManagerProps> = ({
  ...
  const [editItem, setEditItem] = useState<PotTransfer | null>(null);
  ...
  const openAddModal = (preset?: 'isa_to_sipp' | 'gia_to_isa' | 'cash_to_isa' | 'couple_isa' | 'couple_sipp') => {
    ...
    setEditItem(newTransfer);
    setIsAdding(true);
  };

  const openEditModal = (transfer: PotTransfer) => {
    setEditItem({ ...transfer });
    setIsAdding(false);
  };
    ...
    <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 shadow-xs border border-slate-200 dark:border-slate-800 space-y-6 transition-colors">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-slate-800">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-2xl bg-primary-100 dark:bg-primary-950 text-primary-800 dark:text-primary-200 flex items-center justify-center shrink-0">
            <ArrowRightLeft className="w-5 h-5 text-primary-700 dark:text-primary-400" />
          </div>
          <div>
            <h2 className="font-extrabold text-slate-800 dark:text-slate-100 text-base flex items-center gap-2">
              ...
              {!isStudioMode && (
                <span className="text-[10px] font-bold uppercase tracking-wider bg-primary-100 dark:bg-primary-950 text-primary-900 dark:text-primary-200 px-2.5 py-0.5 rounded-full border border-primary-200/50 dark:border-primary-800/50">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3.5 py-2 bg-primary-600 hover:bg-primary-700 text-white text-xs font-bold rounded-xl transition-all shadow-xs cursor-pointer shrink-0 self-start sm:self-auto"
        >
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button
            onClick={() => openAddModal('isa_to_sipp')}
          ...
          <button
            onClick={() => openAddModal('gia_to_isa')}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-semibold rounded-xl transition-all border border-slate-200 dark:border-slate-700 cursor-pointer"
          >
          ...
          <button
            onClick={() => openAddModal('cash_to_isa')}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-semibold rounded-xl transition-all border border-slate-200 dark:border-slate-700 cursor-pointer"
          >
          ...
          {isCouple && (
            <>
              <button
                onClick={() => openAddModal('couple_isa')}
              ...
              <button
                onClick={() => openAddModal('couple_sipp')}
            ...
            return (
              ...
              >
                <div className="space-y-4">
                  ...
                  {/* Top Bar: Name, Details, Edit, Delete */}
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-200/80 dark:border-slate-700/80">
                    <div className="flex flex-wrap items-center gap-3 flex-1">
                      ...
                      <div className="text-[11px] font-semibold text-slate-500 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-md flex items-center gap-1.5 border border-slate-200 dark:border-slate-700">
                        <span>{getPotLabel(transfer.sourcePot)}</span>
                        <ArrowRightLeft className="w-3 h-3" />
                        <span>{getPotLabel(transfer.destinationPot)}</span>
                      </div>
                      <div className="text-xs font-bold text-primary-600 dark:text-primary-400 bg-primary-50 dark:bg-primary-900/30 px-2 py-0.5 rounded-md border border-primary-200 dark:border-primary-800/50">
                    ...
                    <div className="flex items-center gap-3 self-end sm:self-auto shrink-0">
                      <label className="inline-flex items-center gap-2 cursor-pointer text-xs font-semibold text-slate-600 dark:text-slate-300">
                        <input
                          ...
                          className="w-4 h-4 rounded-md text-primary-600 focus:ring-primary-500 bg-slate-200 dark:bg-slate-700 border-slate-300 dark:border-slate-600 cursor-pointer"
                        />
                  ...
                  {!isStudioMode && (
                    <div className="bg-slate-100 dark:bg-slate-900 text-slate-900 dark:text-white p-3.5 sm:p-4 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-3 shadow-md transition-colors">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        ...
                        {/* DESTINATION POT BOX */}
                        <div className="bg-white dark:bg-slate-950/80 p-3.5 rounded-xl border border-primary-200 dark:border-primary-900/40 space-y-2">
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 pb-1 border-b border-primary-100 dark:border-primary-900/30">
                            <span className="text-[11px] font-bold uppercase tracking-wider text-primary-600 dark:text-primary-400 flex items-center gap-1.5">
                          ...
                          <div className="space-y-1.5 text-xs">
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-0.5 text-primary-600 dark:text-primary-400 font-bold">
                              ...
                              <div className="sm:text-right">
                                ...
                                {transfer.destinationPot === 'sipp' && taxReliefAmount > 0 && (
                                  <span className="block text-[10px] text-primary-600 dark:text-primary-300 font-normal">
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-0.5 text-slate-800 dark:text-slate-200 font-bold pt-1.5 border-t border-slate-200 dark:border-slate-800">
                              ...
                              <strong className="text-primary-600 dark:text-primary-400 text-sm sm:text-right">£{Math.round(dstBalanceAfter).toLocaleString()}</strong>
                            </div>
                          </div>
                        </div>
      ...
      {editItem && (
        <ModalShell
          title={isAdding ? 'Add Pot Transfer' : 'Edit Pot Transfer'}
          onSave={handleSave}
          onCancel={handleCancel}
          saveLabel={isAdding ? 'Add Transfer' : 'Save Changes'}
        >
          <div className="space-y-4 text-xs">
            {/* Transfer Name */}
            <div className="space-y-1">
              <label className="font-bold text-slate-600 dark:text-slate-400">Transfer Name</label>
              <input
                type="text"
                value={editItem.name}
                onChange={(e) => setEditItem({ ...editItem, name: e.target.value })}
                className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-100 rounded-xl px-3 py-2 outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500 font-semibold"
                placeholder="e.g. Annual ISA Top Up"
              />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                <label className="font-bold text-slate-600 dark:text-slate-400 flex items-center justify-between">
                  ...
                  {isCouple && (
                    <span className="text-[10px] text-primary-600 dark:text-primary-400 font-extrabold uppercase">
                      {editItem.owner === 'partner' ? partnerName : primaryName}
                    </span>
                  )}
                </label>
                <div className="flex flex-col sm:flex-row gap-1">
                  {isCouple && (
                    <select
                      ...
                      onChange={(e) => setEditItem({ ...editItem, owner: e.target.value as 'primary' | 'partner' })}
                      className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-100 rounded-xl px-2 py-2 focus:border-primary-500 focus:ring-1 focus:ring-primary-500 outline-none w-full sm:w-auto shrink-0 font-medium"
                    >
                  ...
                  <select
                    value={editItem.sourcePot}
                    onChange={(e) => setEditItem({ ...editItem, sourcePot: e.target.value as NonPensionPotType })}
                    className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-100 rounded-xl px-3 py-2 font-semibold focus:border-primary-500 focus:ring-1 focus:ring-primary-500 outline-none"
                  >
                ...
                <label className="font-bold text-slate-600 dark:text-slate-400 flex items-center justify-between">
                  ...
                  {isCouple && (
                    <span className="text-[10px] text-indigo-600 dark:text-indigo-400 font-extrabold uppercase">
                      {editItem.destinationOwner === 'partner' ? partnerName : primaryName}
                    </span>
                  )}
                </label>
                <div className="flex flex-col sm:flex-row gap-1">
                  {isCouple && (
                    <select
                      value={editItem.destinationOwner || editItem.owner || 'primary'}
                      onChange={(e) => setEditItem({ ...editItem, destinationOwner: e.target.value as 'primary' | 'partner' })}
                      className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-100 rounded-xl px-2 py-2 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none w-full sm:w-auto shrink-0 font-medium"
                    >
                      <option value="primary">{primaryName}</option>
                      <option value="partner">{partnerName}</option>
                    </select>
                  ...
                  <select
                    value={editItem.destinationPot}
                    onChange={(e) => setEditItem({ ...editItem, destinationPot: e.target.value as DestinationPotType })}
                    className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-100 rounded-xl px-3 py-2 font-semibold focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none"
                  >
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-100 rounded-xl pl-7 pr-3 py-2 font-bold focus:border-primary-500 focus:ring-1 focus:ring-primary-500 outline-none"
                  />
                </div>
              </div>
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  value={editItem.transferDate || `${currentYear + 1}-04-06`}
                  onChange={(e) => setEditItem({ ...editItem, transferDate: e.target.value })}
                  className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-100 rounded-xl px-3 py-2 font-semibold focus:border-primary-500 focus:ring-1 focus:ring-primary-500 outline-none"
                />
              </div>
            </div>
            ...
            <div className="space-y-1 pt-2">
              ...
              <textarea
                ...
                rows={2}
                className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-100 rounded-xl px-3 py-2 outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500 font-medium resize-none"
            ...
            <div className="flex items-center gap-2 pt-2">
              <label className="inline-flex items-center gap-2 cursor-pointer text-xs font-semibold text-slate-600 dark:text-slate-300">
                <input
                  ...
                  className="w-4 h-4 rounded-md text-primary-600 focus:ring-primary-500 bg-slate-200 dark:bg-slate-700 border-slate-300 dark:border-slate-600 cursor-pointer"
                />
                <span>{editItem.enabled ? 'Active (Transfer will be executed)' : 'Disabled (Draft/Paused)'}</span>
              </label>
            </div>
          </div>
        </ModalShell>
