    ...
    <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 shadow-sm border border-slate-200/80 dark:border-slate-800 space-y-6 transition-colors">
      ...
      <div className="space-y-6">
        {/* Controls Bar for Beneficiary & Exemptions */}
          ...
          {/* Milestone Estate Valuation Summary Cards (Age 80, Age 90, Age 100) */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[milestone80, milestone90, milestone100].map((m) => (
              <div
                key={m.age}
                className={`p-4.5 rounded-2xl border transition-all space-y-3.5 ${
                  m.netIhtLiability > 0
                    ? 'bg-gradient-to-b from-rose-50/50 to-amber-50/30 dark:from-rose-950/20 dark:to-amber-950/10 border-rose-200 dark:border-rose-900/60'
                    : 'bg-gradient-to-b from-emerald-50/50 to-teal-50/30 dark:from-emerald-950/20 dark:to-teal-950/10 border-emerald-200 dark:border-emerald-900/60'
                }`}
              >
                {/* Header */}
                <div className="flex items-center justify-between pb-2 border-b border-slate-200/60 dark:border-slate-800">
                  <div>
                    ...
                    <span className="text-[10px] text-slate-500 dark:text-slate-400">
                      {m.age === 100 ? 'Century Horizon' : `In ${m.age - currentAge} Years`}
                    </span>
                  </div>
                  <span
                  ...
                  >
                    {m.netIhtLiability > 0 ? `IHT Bill: £${m.netIhtLiability.toLocaleString()} (${m.isCharityRateUnlocked ? '36%' : '40%'})` : '0% IHT (Under Allowances)'}
                  </span>
                </div>
                ...
                <div className="space-y-1.5 text-xs">
                  <div className="flex items-center justify-between text-slate-600 dark:text-slate-300">
                    ...
                    <span className="font-bold text-slate-900 dark:text-white">£{m.propertyValue.toLocaleString()}</span>
                  </div>
                  <div className="flex items-center justify-between text-slate-600 dark:text-slate-300">
                    <span>ISAs & Financial Wealth:</span>
                    <span className="font-bold text-slate-900 dark:text-white">£{m.grossNonPensionWealth.toLocaleString()}</span>
                  </div>
                  {m.bprExemptionDeduction > 0 && (
                    <div className="flex items-center justify-between text-emerald-600 dark:text-emerald-400 text-[11px] font-semibold">
                      <span>Business Relief (BPR/AIM) Exempt:</span>
                      <span>-£{m.bprExemptionDeduction.toLocaleString()}</span>
                    </div>
                  )}
                  <div className="flex items-center justify-between text-slate-600 dark:text-slate-300">
                    <span>Unused Pension Wealth:</span>
                    <span className={`font-bold ${iht.includePensionsInEstate ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-400 line-through'}`}>
                      £{m.pensionWealth.toLocaleString()}
                    </span>
                  </div>
                  {m.cumulativeAnnualGifting > 0 && (
                    <div className="flex items-center justify-between text-emerald-600 dark:text-emerald-400 text-[11px] font-semibold">
                      <span>Cumulative Gifting Deducted:</span>
                      <span>-£{m.cumulativeAnnualGifting.toLocaleString()}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="pt-2 border-t border-slate-200/60 dark:border-slate-800 flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-700 dark:text-slate-300">Gross Taxable Estate:</span>
                  <span className="text-sm font-black text-slate-900 dark:text-white">£{m.grossEstateValuation.toLocaleString()}</span>
                </div>

                {/* Allowances & Exemptions */}
                <div className="bg-white/80 dark:bg-slate-900/80 p-2.5 rounded-xl border border-slate-200/60 dark:border-slate-800 space-y-1 text-[11px]">
                  <div className="flex items-center justify-between text-slate-600 dark:text-slate-400">
                    <span>Nil Rate Band (NRB):</span>
                    <span className="font-bold">£{m.baseNrb.toLocaleString()}</span>
                  </div>
                  <div className="flex items-center justify-between text-slate-600 dark:text-slate-400">
                    <span>Residence NRB (RNRB):</span>
                    <span className="font-bold">£{m.baseRnrb.toLocaleString()}</span>
                  </div>
                  {m.charityDeduction > 0 && (
                    <div className="flex items-center justify-between text-rose-600 dark:text-rose-400 font-bold">
                      <span>Charitable Bequest Deduction:</span>
                      <span>£{m.charityDeduction.toLocaleString()}</span>
                    </div>
                  )}
                  <div className="flex items-center justify-between font-extrabold text-indigo-700 dark:text-indigo-300 pt-1 border-t border-slate-200 dark:border-slate-800">
                    <span>Total Tax-Free Threshold:</span>
                    <span>£{(m.totalAllowances + m.charityDeduction).toLocaleString()}</span>
                  </div>
                </div>

                {/* Final Net Result Passed to Heirs */}
                <div className="pt-2 border-t border-slate-200 dark:border-slate-800 space-y-1">
                  <div className="flex items-center justify-between text-xs">
                    <span className="font-extrabold text-rose-600 dark:text-rose-400">
                      Estimated IHT Tax ({m.isCharityRateUnlocked ? '36%' : '40%'}):
                    </span>
                    <span className="font-black text-rose-600 dark:text-rose-400">£{m.netIhtLiability.toLocaleString()}</span>
                  </div>
                  {m.insurancePayout > 0 && (
                    <div className="flex items-center justify-between text-xs text-indigo-600 dark:text-indigo-400 font-semibold">
                      <span>Life Policy in Trust Offset:</span>
                      <span>-£{m.insurancePayout.toLocaleString()}</span>
                    </div>
                  )}
                  <div className="flex items-center justify-between text-xs pt-1">
                    <span className="font-extrabold text-emerald-700 dark:text-emerald-300">Net Wealth Passed to Heirs:</span>
                    <span className="text-sm font-black text-emerald-700 dark:text-emerald-300">£{m.netPassedToHeirs.toLocaleString()}</span>
                  </div>
                </div>
